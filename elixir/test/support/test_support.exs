defmodule SymphonyElixir.TestSupport do
  ...
  defp workflow_content(overrides) do
    config =
      Keyword.merge(
        [
          ...
          agent_gh_token_keychain: nil,
          agent_env_passthrough: nil,
    ...
    tracker_scope = Keyword.get(config, :tracker_scope)
    tracker_filter_label = Keyword.get(config, :tracker_filter_label)
    ...
    agent_gh_token_keychain = Keyword.get(config, :agent_gh_token_keychain)
    agent_env_passthrough = Keyword.get(config, :agent_env_passthrough)
    ...
    sections =
      [
        ...
        optional_yaml_field("  projects:", tracker_projects),
        optional_yaml_field("  scope:", tracker_scope),
        optional_yaml_field("  filter_label:", tracker_filter_label),
        "polling:",
        ...
        optional_yaml_field("  gh_token_keychain:", agent_gh_token_keychain),
        optional_yaml_field("  env_passthrough:", agent_env_passthrough),
        "codex:",
  ...
  defp optional_yaml_field(_label, nil), do: nil
  defp optional_yaml_field(label, value), do: "#{label} #{yaml_value(value)}"

  defp hooks_yaml(nil, nil, nil, nil, nil, timeout_ms), do: "hooks:\n  timeout_ms: #{yaml_value(timeout_ms)}"

  defp hooks_yaml(hook_after_create, hook_before_run, hook_before_turn, hook_after_run, hook_before_remove, timeout_ms) do
    [
      "hooks:",
      "  timeout_ms: #{yaml_value(timeout_ms)}",
